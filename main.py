...
client = commands.Bot(command_prefix='=', owner_id = 0, case_insensitive=True, intents=intents, activity=discord.Activity(type=discord.ActivityType.watching, name="=help"))
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        ...
        if cmd := ctx.bot.get_command(command): 
            await ctx.send(f"Usage: {client.help_command.get_usage(cmd)}\nFor more information, please  use `=help {command}`.")
